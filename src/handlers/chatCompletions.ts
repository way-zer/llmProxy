...
function extractTokens(chunk: Record<string, unknown>): Tokens | null {
  ...
  const d = (usage.prompt_tokens_details ?? {}) as Record<string, unknown>;
  return {
    ...
    cacheWrite: (d.cache_creation_tokens ?? 0) as number,
  };
}
...
export async function handleChatCompletions(req: Request): Promise<Response> {
  ...
  try {
    ...
    if (!logBase.stream) {
      ...
      try { const p = JSON.parse(t); tokens = extractTokens(p) ?? ZERO_TOKENS; const { choices, ...m } = p; meta = m; } catch { /* */ }
    ...
    return pipeStream(res,
      chunk => {
        ...
        if (t) {
          tokens.input = t.input || tokens.input;
          tokens.output = t.output || tokens.output;
          tokens.reasoning = t.reasoning || tokens.reasoning;
          tokens.cached = t.cached || tokens.cached;
          tokens.cacheWrite = t.cacheWrite || tokens.cacheWrite;
        }
      },
      () => {
